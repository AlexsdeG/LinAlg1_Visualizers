  ...
  useEffect(() => {
    ...
    switch (activeOp) {
      ...
      case '/':
        ...
        // Division only highlights if we switch grid to division table
        if (activeOp === '/') {
          setActiveRow(a);
          setActiveCol(b);
        }
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              <button
                onClick={() => setActiveOp('+')}
                className={`p-3 rounded font-bold transition-colors ${activeOp === '+' ? 'bg-blue-600 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}
              >
                +
              </button>
              <button
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
            <div className="flex flex-col gap-4 mb-4">
              <div className="flex justify-between items-center">
                ...
                <div className="flex flex-col items-end gap-1">
                  <div className="text-xs text-slate-500 bg-green-50 text-green-700 px-2 py-1 rounded border border-green-100">
                    Green = {activeOp === '*' ? '1 (Identity/Inverse)' : 'Target Value'}
                  </div>
                  <div className="text-xs text-slate-500 bg-red-50 text-red-700 px-2 py-1 rounded border border-red-100">
              ...
              {/* Grid Tabs */}
              <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit self-center">
                {['+', '-', '*', '/'].map((op) => (
                  <button
                    key={op}
                    onClick={() => {
                      setActiveOp(op as any);
                      setActiveRow(null);
                      setActiveCol(null);
                    }}
                    className={`px-4 py-1.5 rounded-md font-bold text-lg transition-all ${activeOp === op
            ...
            <ModuloGrid
              ...
              highlightNumber={activeOp === '*' ? 1 : null}
              activeRow={activeRow}
              activeCol={activeCol}
              operation={activeOp}
            />
